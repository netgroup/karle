...
int main(int argc, char *argv[]) {
  ...
  int i, k, num_packet_sent, optval;
  double t1, t2;
  ...
  // Bind address and then in listening
  ...
  // Connections OK. Send 100 packet of size 1 byte
  // and for each send wait for ack
  t1 = 0.0;
  t2 = 0.0;
  ...
  for(num_packet_sent=0; num_packet_sent<100; num_packet_sent++){
    ...
    if(optval==-1) {
      ...
      t1 += start.tv_sec+(start.tv_usec/1000.0);
      t2 += end.tv_sec+(end.tv_usec/1000.0);
    }
  }
